...
class CommandConsole:
    def __init__(self, client_id):
        ...
        try:
            with ui.column().classes("w-full bg-gray-700 text-white").style(
                "height: 96vh; overflow: hidden;"
            ):

                # Header or navbar area at the top
                with ui.row().classes(
                    "w-full bg-gray-800 p-2 items-center justify-between"
                ):
                    ui.label(f"Command Console - Client ID: {self.client_id}").classes(
                        "text-lg text-white"
                    )
                    ...
                    ).classes("bg-blue-500 text-white px-4 py-1 rounded-sm")

                    # cmd mode
                    checkbox = ui.checkbox(
                        "CMD Mode",
                        on_change=lambda: self.set_checkbox_state(checkbox.value),
                    )
                ...
                self.output_area = (
                    ui.scroll_area()
                    .classes("flex-grow p-4 space-y-4 overflow-hidden")
                    .style(
                ...
                with ui.row().classes("w-full bg-gray-800 p-2 items-center"):
                    self.command_input = (
                        ui.input(placeholder="Enter command...")
                        .classes(
                    ...
                    self.run_button = ui.button(
                        ...
                        color=ThemeConfig().get_button_color(),
                    ).classes("bg-blue-500 text-white ml-2 px-4 py-1 rounded-sm")

                    self.command_input.on(
                        "keydown.enter", lambda _: self.on_run_click()
                    )
        ...
        try:
            ...
            if command:
                self.command_input.value = (
                    ""  # Clear the input field - needs to go first
                )
    ...
    def execute_command(self, command):
        ...
        try:
            ...
            post_url = ThemeConfig().get_url() / "command" / self.client_id
            logger.debug(f"Posting command to {post_url}")

            # Send to correct endpoint to queue message
            headers = {"Authorization": f"Bearer {ThemeConfig().get_token()}"}
            ...
            self.timers[rid] = ui.timer(
                interval=2, callback=lambda: self.check_response(rid)
            )
    ...
    def set_checkbox_state(self, value: bool):
        ...
        if value:
            ui.notify(
                "CMD mode enabled, every entry will be prefixed with 'command' - so it's basically a real shell"
            )
    ...
    def check_response(self, rid):
        ...
        try:
            ...
            response_url = ThemeConfig().get_url() / "response" / rid
            logger.debug(f"checking for response at {response_url}")

            headers = {"Authorization": f"Bearer {ThemeConfig().get_token()}"}

            response = requests.get(
                response_url, headers=headers, verify=ThemeConfig().get_verify_certs()
            )
            if response.status_code == 200:
    ...
    def display_output(self, output):
        ...
        try:
            ...
            with self.output_area:
                ui.label(output).classes("text-sm text-white").style(
