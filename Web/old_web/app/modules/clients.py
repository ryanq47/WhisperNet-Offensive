...
def clients():
    try:
        ...
        with ui.column().classes("h-auto w-full"):  # add border to show border
            ...
            grid.classes(
                "flex-grow"
            )  # w-full h-full')#.style('width: 100%; height: 95vh')
            ui.chat_message(
                "This AgGrid is being frustrating... and does not want to scale properly, or have colored themes apply to it"
            )
        ...
        def fetch_and_update_grid_data():
            ...
            try:
                ...
                for data in client_data:
                    utc_checkin = datetime.fromtimestamp(
                        int(data["checkin"]), timezone.utc
                    )
                    formatted_checkin = utc_checkin.astimezone().strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )

                    client_id = data["client_id"]
                    grid.run_row_method(
                        client_id, "setDataValue", "checkin", formatted_checkin
                    )
...
def get_client_data() -> dict:
    ...
    """
    try:
        url = ThemeConfig().get_url() / "stats" / "clients"
        ...
        if response.status_code == 200:
            ...
            logger.debug(
                f"Client data retrieved: {client_data}"
...
def transform_client_data(raw_data):
    ...
    logger.debug(
        f"Transformed client data: {transformed_data}"
