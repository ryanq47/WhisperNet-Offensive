...
def simple_http_queue_command(client_id):
    try:
        ...
        # set up queue
        queue = RedisQueue(client_id=client_id)
        # enque this request ID to client queue
        queue.enqueue(item=fj.rid)
        ...
        dict_data["audit"][
            "user_identity"
        ] = user_identity  # Add user identity to the log

        a = Audit()
        a.audit(dict_data)

        return api_response(status=200, message="Command queued successfully")
...
@app.route(
    "/response/<response_id>", methods=["GET"], endpoint="simple_http_get_response"
)
@jwt_required()
def simple_http_get_response(response_id):
    ...
    try:
        ...
        if response_redis == None:
            ...
            return api_response(status=404, message="Response not found")
        ...
        return api_response(rid=rid, data=data)
...
def simple_http_get(client_id):
    ...
    try:
        ...
        rq = RedisQueue(client_id=client_id)
        ...
        client = Agent(
            type="simple-http", client_id=client_id, checkin=int(time.time())
        )
        client.save()

        # send back to client - WITH rid
        return api_response(data=command_dict.data, rid=rid_of_command)
...
def simple_http_post(client_id):
    try:
        ...
        return api_response(status=200, message="Request stored successfully")
