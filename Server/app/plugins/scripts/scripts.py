...
script_ns = Namespace(
    "Plugin: scripts",
    description="Endpoints for managing, getting agent scripts",
)
...
# ------------------------------------------------------------------------------------
script_response = script_ns.model(
    "StaticServeResponse",
    {
...
# ------------------------------------------------------------------------------------
upload_parser = script_ns.parser()
...
class StaticServeUploadResource(Resource):
    ...
    # @jwt_required
    @script_ns.marshal_with(script_response, code=200)
    # @jwt_required()
    def post(self):
        ...
        static_dir = os.path.join(Config().root_project_path, "data/scripts/")
...
# Delete File
# ------------------------------------------------------------------------------------
@script_ns.route("/delete/<path:filename>")
...
# ------------------------------------------------------------------------------------
# @script_ns.route("")
# @script_ns.doc(description="A simple health-check or ping endpoint.")
...
Instance().api.add_namespace(script_ns, path="/scripts")
